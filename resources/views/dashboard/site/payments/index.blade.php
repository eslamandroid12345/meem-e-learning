                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="card-tools">
                                <a href="{{ route('payments.exportPayments').'/excel?'.http_build_query(request()->query()) }}"
                                   class="btn  btn-success">@lang('dashboard.Excel Export')</a>
                                <a href="{{ route('payments.exportPayments').'/pdf?'.http_build_query(request()->query()) }}"
                                   class="btn  btn-danger">@lang('dashboard.PDF Export')</a>
                            </div>
                        </div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-2">
                                        <select id="type" name="type" class="form-control">
                                            <option
                                                @selected(request('type') == "ALL") value="ALL">@lang('dashboard.all_items')</option>
                                            <option
                                                @selected(request('type') == "COURSE") value="COURSE">@lang('dashboard.courses')</option>

                                            <option
                                                @selected(request('type') == "COURSES") value="COURSES">@lang('dashboard.courses_select')</option>

                                            <option
                                                @selected(request('type') == "BOOK")  value="BOOK">@lang('dashboard.books')</option>
                                            <option
                                                @selected(request('type') == "CERTIFICATE") value="CERTIFICATE">@lang('dashboard.certificates')</option>
                                        </select>
                                    </div>

                                    <div style="display: none" class="form-group col-2 all-courses">
                                        <select id="course_id" name="course_id" class="form-control">
                                            <option value="" selected>@lang('dashboard.courses_selected')</option>
                                            @foreach($courses as $course)
                                                <option @selected(request('course_id') == $course->id) value="{{$course->id}}">{{app()->getLocale() == 'ar' ? $course->name_ar : $course->name_en}}</option>
                                    ...
                                    <div class="form-group col-2">
                                        <select id="payment_type" name="payment_type" class="form-control">
                                            <option
                                                @selected(request('payment_type') == "ALL") value="ALL">@lang('dashboard.payment_methods')</option>
                                            <option
                                    ...
                                    <div class="form-group col-2">
                                        <input value="{{request('search') ?? ""}}" name="search" type="search"
                                               class="form-control"
                                               placeholder="@lang('dashboard.search_with_phone_number')">
                                    </div>
                                    ...
                                    <div class="form-group col-2">
                                        <input value="{{request('from_date') ?? ""}}" name="from_date" type="date"
                                               class="form-control"
                                               placeholder="@lang('dashboard.search_with_phone_number_or_name_or_email')">
                                    </div>
                                            ...
                                            <div class="operations-btns" style="">
                                                ...
                                                @if(Gate::allows('confirm-payment', $payment) && auth()->user()->hasPermission('payments-update'))
                                                    <button class="btn btn-dark waves-effect waves-light"
                                                            data-toggle="modal"
                                                            data-target="#confirm-modal-{{$key}}">@lang('dashboard.Confirm Payment')</button>
                                                    <div id="confirm-modal-{{$key}}" class="modal fade modal2 "
                                                        ...
                                                        <div class="modal-dialog">
                                                            <div class="modal-content float-left">
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <form
                                                                        action="{{route('payments.confirm' , $payment['id'])}}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('PUT')
                                                                        <button type="submit"
                                                                                class="btn btn-success">@lang('dashboard.Confirm')</button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                @if(Gate::allows('decline-payment', $payment) && auth()->user()->hasPermission('payments-update'))
                                                    <button class="btn btn-danger waves-effect waves-light"
                                                            data-toggle="modal"
                                                            data-target="#decline-modal-{{$key}}">@lang('dashboard.Decline Payment')</button>
                                                    <div id="decline-modal-{{$key}}" class="modal fade modal2 "
                                                         tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
                                                         aria-hidden="true" style="display: none;">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content float-left">
                                                                ...
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <form
                                                                        action="{{route('payments.decline' , $payment['id'])}}"
                                                                        method="post">
                                                                        @csrf
                                                                        @method('PUT')
                                                                        <button type="submit"
                                                                                class="btn btn-danger">@lang('dashboard.Decline')</button>
                                                                    </form>
                                                                </div>
                                                            </div>
                                                        </div>
