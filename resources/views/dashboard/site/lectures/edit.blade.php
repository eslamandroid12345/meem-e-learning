...
@section('title', __('titles.Edit') . " " . __('titles.Lecture'))
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <form action="{{ route('lectures.update', $lecture->id) }}" method="post" autocomplete="off"
                              enctype="multipart/form-data" novalidate>
                            <div class="card-header">
                                <h3 class="card-title">{{__('titles.Edit') . " " . __('titles.Lecture')}}</h3>
                            </div>
                            <div class="card-body">
                                @method('PUT')
                                @csrf
                                <div class="row">
                                    <div class="form-group col-6">
                                        ...
                                        <select id="type" name="type" class="form-control">
                                            <option
                                                @selected(old('type') !== null ? old('type') == 'RECORDED' : $lecture->type == 'RECORDED') value="RECORDED">@lang('db.lecture_type.RECORDED')</option>
                                            <option
                                                @selected(old('type') !== null ? old('type') == 'LIVE' : $lecture->type == 'LIVE') value="LIVE">@lang('db.lecture_type.LIVE')</option>
                                        </select>
                                    </div>
                                    <div class="form-group col-3">
                                        ...
                                        <select id="course" name="course_id" class="select2 select2-hidden-accessible" data-placeholder="Select a State" style="width: 100%;" data-select2-id="2" tabindex="-1" aria-hidden="true">
                                            ...
                                            @foreach($courses as $course)
                                                <option @selected((old('course_id') ?? $lecture->standard?->course_id )== $course['id']) value="{{$course['id']}}">{{$course->t('name')}}</option>
                                            @endforeach
                                        </select>
                                    </div>
...
{{--                                                        <option--}}
{{--                                                            @selected(old('standard_id') !== null ? old('standard_id') == $standard['id'] : $standard['id'] == $lecture->standard_id) value="{{$standard['id']}}">{{$standard->t('name')}}</option>--}}
{{--                                                    @endforeach--}}
{{--                                                </optgroup>--}}
{{--                                            @endforeach--}}
{{--                                        </select>--}}
{{--                                    </div>--}}
                                ...
                                <div class="row">
                                    <div class="form-group col-6">
                                        ...
                                        <input name="name_ar" type="text" class="form-control" id="exampleInputName1"
                                               value="{{ old('name_ar') ?? $lecture->name_ar }}" placeholder="">
                                    </div>
                                    <div class="form-group col-6">
                                        ...
                                        <input name="name_en" type="text" class="form-control" id="exampleInputName1"
                                               value="{{ old('name_en') ?? $lecture->name_en }}" placeholder="">
                                    </div>

                                    <div class="form-group col-6">
                                        <label for="summernote">@lang('dashboard.Description') @lang('dashboard.ar')</label>
                                        <textarea  name="description_ar" type="text" class="form-control" id="summernote">{{ old('description_ar') ?? $lecture['description_ar'] }}</textarea>
                                    </div>

                                    <div class="form-group col-6">
                                        <label for="summernote2">@lang('dashboard.Description') @lang('dashboard.en')</label>
                                        <textarea  name="description_en" type="text" class="form-control" id="summernote2">{{ old('description_en') ?? $lecture['description_en'] }}</textarea>
                                    </div>
                                    ...
                                    <div class="row">
                                        @if(auth()->user()->hasPermission('lectures-links'))
                                            <div class="col-2">
                                                ...
                                                <select id="platform_type" class="form-control" name="link_platform">
                                                    <option @selected($lecture->link_platform == "YOUTUBE") value="YOUTUBE">@lang('dashboard.youtube')</option>
                                                    <option @selected($lecture->link_platform == "VIMEO") value="VIMEO">@lang('dashboard.vimeo')</option>
                                                    <option @selected($lecture->link_platform == "SWARMIFY") value="SWARMIFY">@lang('dashboard.swarmify')</option>
                                                </select>
                                            </div>
                                            <div class="form-group col-6">
                                            <label
                                            ...
                                            <input name="record_link" type="text" class="form-control"
                                                   id="exampleInputRecordedLink" value="{{ old('record_link') ?? $lecture->record_link }}"
                                                   placeholder="">
                                        </div>
                                        @endif
                                        <div class="form-group col-4">
                                            ...
                                            <input name="publish_at" type="datetime-local" class="form-control" id="publish_at"
                                                   value="{{ old('publish_at') ?? $lecture->publish_at }}"/>
                                        </div>
                                    </div>
                                    <div class="row">
                                        @if(auth()->user()->hasPermission('lectures-links'))
                                            <div class="form-group col-6">
                                            <label
                                                for="exampleInputRecordedLink">@lang('dashboard.Live Lecture Link')</label>
                                            <input name="live_link" type="text" class="form-control"
                                                   id="exampleInputLiveLink" value="{{ old('live_link') ?? $lecture->live_link }}"
                                                   placeholder="">
                                        </div>
                                        @endif
                                        <div class="form-group col-3">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group clearfix col-3">
                                        ...
                                        <input name="sort" type="number" min="1" step="1" class="form-control" id="exampleInputName1" placeholder="" value="{{old('sort') ?? $lecture->sort}}">
                                    </div>
                                    <div class="clearfix col-3" style="display: grid;">
                                        <div class="row align-items-end">
                                            <div class="form-group col-6 m-0">
                                                <input name="is_free" type="hidden" value="off">
                                                <div class="icheck-wetasphalt d-inline">
                                                    <input name="is_free" type="checkbox" id="checkboxPrimary3" @checked(old('is_free') !== null ? old('is_free') == 'on' : $lecture->is_free)>
                                            ...
                                            <div class="form-group col-6 m-0">
                                                ...
                                                <div class="icheck-wetasphalt d-inline">
                                                    <input name="is_active" type="checkbox" id="checkboxPrimary4" @checked(old('is_active') !== null ? old('is_active') == 'on' : $lecture->is_active)>
...
@section('js_addons')
    ...
    <script>
        $(function () {
            let pin_count = 0;
            @if(old('pins') !== null || $lecture->pins !== null)
            @foreach(old('pins') ?? $lecture->pins as $key => $pin)
            const old_pin_{{$key}} = '' +
                '<div class="row pin">' +
                '    <div class="form-group col-2">' +
                '        <label for="exampleInputName1">@lang("dashboard.Pin Time")</label>' +
                '        <input name="pins[' + pin_count + '][time]" type="text" class="form-control" id="exampleInputName1" value="{{ old('pins.'.$key.'.time') ?? $pin['time'] }}" placeholder="hh:mm:ss">' +
                '    </div>' +
                '    <div class="form-group col-2">' +
                '        <label for="exampleInputName1">@lang("dashboard.Pin Name Ar")</label>' +
                '        <input name="pins[' + pin_count + '][name_ar]" type="text" class="form-control" id="exampleInputName1" value="{{ old('pins.'.$key.'.name_ar') ?? $pin['name_ar'] }}" placeholder="">' +
                '    </div>' +
                '    <div class="form-group col-2">' +
                '        <label for="exampleInputName1">@lang("dashboard.Pin Name En")</label>' +
                '        <input name="pins[' + pin_count + '][name_en]" type="text" class="form-control" id="exampleInputName1" value="{{ old('pins.'.$key.'.name_en') ?? $pin['name_en'] }}" placeholder="">' +
                '    </div>' +
                '    <div class="col-1">' +
    ...
    <script>
        $(function () {
            ...
            @if(old('indicators') !== null || $lecture->indicators !== null)
            @foreach(old('indicators') ?? $lecture->indicators as $key => $indicator)
            const old_indicator_{{$key}} = '' +
                '<div class="row indicator">' +
                '    <div class="form-group col-5">' +
                '        <label for="exampleInputName1">@lang("dashboard.Indicator Name Ar")</label>' +
                '        <input name="indicators[' + indicator_count + '][name_ar]" type="text" class="form-control" id="exampleInputName1" value="{{ old('indicators.'.$key.'.name_ar') ?? $indicator['name_ar'] }}" placeholder="">' +
                '    </div>' +
                '    <div class="form-group col-5">' +
                '        <label for="exampleInputName1">@lang("dashboard.Indicator Name En")</label>' +
                '        <input name="indicators[' + indicator_count + '][name_en]" type="text" class="form-control" id="exampleInputName1" value="{{ old('indicators.'.$key.'.name_en') ?? $indicator['name_en'] }}" placeholder="">' +
                '    </div>' +
                '    <div class="col-2">' +
...
{{--                    $('#standard').append('<option value="0">@lang('dashboard.none')</option>');--}}
                    ...
                    $.each(data, function (key, value) {
                        $('#standard').append('<option '+ ({{ old('standard_id') ?? $lecture->standard_id }} === value.id ? ' selected ' : '') +' value="' + value.id + '">' + value.name_{{app()->getLocale()}} + '</option>');
                    });
                },
                error: function (data) {
